...
"""
# Comprobación que cumple con la condición de las 7 UIT's
if remunera_bruta > 7 * UIT_2025 :
    remuneracion_neta = remunera_bruta - (7 * UIT_2025)
    imp_anual_proyectado = tasa_impuesto(remuneracion_neta)

    if mes_ingreso > 1:
        ant = int(input("¿Han retenido parte de tu sueldo en tu anterior trabajo? (1.SI    2.NO): "))
        print("")
        if ant == 1:
            # SI LA RESPUESTA ES SI, SE DEBE CALCULAR NUEVAMENTE LOS VALORES DE REMU NETA Y LO QUE VIENE DESPUES Y SACAR CADA MES
            retencion_anterior = float(input("¿Cuánto te han retenido hasta el momento? (S/.) : "))
            print("")
            # Falta implementar



    retencion_mensual = calculo_retencion_mensual(mes_ingreso,imp_anual_proyectado)

    aumento = int(input("¿Te han aumentado el sueldo durante este año?(1.SI   2.NO): ")) 
    print("")
    if aumento == 1:
        sueldo_aumento = float(input("¿Cuál es tu nuevo sueldo?(s/.): "))
        print("")
        mes_aumento = int(input("¿En qué mes recibiste tu aumento?(2 -> Febrero al 12 -> Diciembre): ")) 
        print("")
        sueldo_nuevo = sueldo_aumento
        mes_au = mes_aumento
        # Aqui tendria que hacer todo el cálculo de un nuevo vector de retenciones, modificando funciones para que esto pueda ser posible
        nuevo_remu_bruta = remuneracion_bruta(sueldo_mes,mes_ingreso,mes_au,sueldo_nuevo)
        nuevo_remu_neta = nuevo_remu_bruta - (7 * UIT_2025)
        new_imp_anual_proy = tasa_impuesto(nuevo_remu_neta)
        retencion_anual = new_imp_anual_proy - imp_anual_proyectado
        new_reten_mensual = retencion_anual / ((12 - mes_au) + 1)
        for i in range(mes_au-1,12):
            retencion_mensual[i] = new_reten_mensual
    


    bono_extra = int(input("¿Ha recibido alguna bonificación o pago extraordinario?(1.SI   2.NO): "))
    print("")
    while bono_extra == 1:
        mes_bono = int(input("¿En qué mes?(1 -> Enero al 12 -> Diciembre): "))
        m_adic = float(input("¿Qué monto? (s/.): "))
        retencion_adicional = retencion_adicional_mes(m_adic,remuneracion_neta,imp_anual_proyectado)
        retencion_mensual[mes_bono-1] = retencion_mensual[mes_bono-1] + retencion_adicional
        bono_extra = int(input("¿Ha recibido otra bonificación o pago extraordinario?(1.SI   2.NO): "))

    print("Tu impuesto anual proyectado es: S/." + str(imp_anual_proyectado))
    print("")
    print("La retención que te corresponde por cada mes es la siguiente.")
    print("")
    imprimir_retenciones(retencion_mensual)

else:
    print("A usted no le corresponde retención de un porcentaje de su sueldo")
